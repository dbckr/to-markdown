...
module.exports = [
...

  {
    ...
    replacement: function (content, node) {
      ...
      if (node.parentNode.nodeName === 'THEAD' || node.childNodes.length > 0 && node.childNodes[0].nodeName === 'TH') {
        for (var i = 0; i < node.childNodes.length; i++) {
...
module.exports = [
  ...
  },
  {
        ...
        replacement: function(innerHTML, node) {
          if (innerHTML)
          {
            if (node.classList.contains('confluence-information-macro-warning'))
              return '(IMPORTANT) ' + innerHTML.trim() + '\n';
            else if (node.classList.contains('confluence-information-macro-note'))
              return '(WARNING) ' + innerHTML.trim() + '\n';
            else if (node.classList.contains('confluence-information-macro-information'))
              return '(NOTE) ' + innerHTML.trim() + '\n';
            else if (node.classList.contains('brush:'))
            {
              var temp = innerHTML.split('\n');
              temp.forEach(function (val, ix) {
                var tabs = val.split('\t').length - 1; 
                temp[ix] = val.trim();
